  ...
  cols: [
    ...
    },
    {
      ...
      click: function () {
        ...
        let date = new Date();
        sendData.theme_id = date.getTime();
        ...
        webix.confirm({
          title: "new theme",
          text: "Are you sure you want to insert this theme?",
          callback: function (result) {
            if (result === true) {
              test_data.map((item) => {
                item.theme_id = sendData.theme_id;
                return item;
              });
              ...
              webix
                ...
                .post("/insert_theme", sendData, function (text, xml, xhr) {
                  if (text == "success") {
                    webix.message({
                      type: "success",
                      text: "New theme added successfully",
                    });
                  }
                });
            } else {
...
function updateTest() {
  webix.confirm({
    ...
    callback: function (result) {
      if (result === true) {
        var index = test_data.findIndex(
          (item) => item.test_id === editable_row.test_id
        );
        if (index != -1) {
          let query = document.getElementById("query").value;
          let query_type = document.getElementById("query_type").value;
